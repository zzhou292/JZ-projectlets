function [problem,guess] = fp(obs)

problem.data.auxdata=obs;


%------------- BEGIN CODE --------------
% Plant model name, used for Adigator
InternalDynamics=@fp_Dynamics_Internal;
SimDynamics=@fp_Dynamics_Sim;


% Settings file
problem.settings=@settings_fp;
...
problem.states.x0=[-18 -20 0];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[-18 -20 0]; 
problem.states.x0u=[-18 -20 0]; 


% State bounds. xl=< x <=xu
problem.states.xl=[-25 -25 0];
problem.states.xu=[25 25 2*pi];
...
problem.states.xfl=[16 21 0]; 
problem.states.xfu=[16 21 0];

% Guess the state trajectories with [x0 xf]
guess.states(:,1)=[-18 16];
guess.states(:,2)=[-20 21];
guess.states(:,3)=[0 0];
...
% Input bounds
problem.inputs.ul=[0 -pi*2];
problem.inputs.uu=[50 pi*2];

% Bounds on first control action
problem.inputs.u0l=[0 0];
problem.inputs.u0u=[0 0]; 
...
temp_gl = zeros(1,2*n_obs);

for i=1:n_obs
    temp_gl(1,2*(i-1)+1) = obs(3,i);
    temp_gl(1,2*(i-1)+2) = obs(3,i);
    problem.constraints.gu(1,2*(i-1)+1) = inf;
    problem.constraints.gu(1,2*(i-1)+2) = inf;
    problem.constraints.gTol_neq(1,2*(i-1)+1)=1e-2;
    problem.constraints.gTol_neq(1,2*(i-1)+2)=1e-2;
end

problem.constraints.gl=temp_gl;
